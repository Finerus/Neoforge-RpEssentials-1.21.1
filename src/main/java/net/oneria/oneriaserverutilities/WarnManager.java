...
public class WarnManager {
    ...
    private static synchronized void ensureInitialized() {
        ...
        try {
            ...
            OneriaServerUtilities.LOGGER.info("[WarnManager] Initialized - File: {}", dataFile.getAbsolutePath());
        } catch (Exception e) {
            OneriaServerUtilities.LOGGER.error("[WarnManager] Failed to initialize", e);
        }
    }
    ...
    private static void loadFromFile() {
        ...
        try (FileReader reader = new FileReader(dataFile)) {
            ...
            if (data != null) {
                ...
                OneriaServerUtilities.LOGGER.info("[WarnManager] Loaded {} warns (next id={})", data.size(), maxId + 1);
            }
        } catch (Exception e) {
            OneriaServerUtilities.LOGGER.error("[WarnManager] Failed to load warns", e);
        }
    }
    ...
    private static void saveToFile() {
        ...
        CompletableFuture.runAsync(() -> {
            try {
                ...
                OneriaServerUtilities.LOGGER.debug("[WarnManager] Saved {} warns", snapshot.size());
            } catch (Exception e) {
                OneriaServerUtilities.LOGGER.error("[WarnManager] Failed to save warns", e);
            }
        });
    }
    ...
    public static String addWarn(UUID targetUUID, String targetName,
        ...
        warns.add(entry);
        saveToFile();
        OneriaServerUtilities.LOGGER.info("[WarnManager] Warn #{} added for {} by {} — reason: {}", id, targetName, issuerName, reason);
        return id;
    }
    ...
    private static void recalculateCounter() {
        ...
        OneriaServerUtilities.LOGGER.debug("[WarnManager] Counter recalculated → next id={}", max + 1);
    }
    ...
    public static boolean removeWarn(String warnId) {
        ...
        if (removed) {
            recalculateCounter();
            saveToFile();
            OneriaServerUtilities.LOGGER.info("[WarnManager] Warn #{} removed", warnId);
        }
        return removed;
    }
    ...
    public static int purgeExpiredWarns() {
        ...
        if (removed > 0) {
            recalculateCounter();
            saveToFile();
            OneriaServerUtilities.LOGGER.info("[WarnManager] Purged {} expired warns", removed);
        }
        return removed;
    }
